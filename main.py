...
#==========================helper funtions================
def information(pred_dis):
    disease_desciption = description[description['Disease'] == predicted_disease]['Description']
    disease_desciption = " ".join([w for w in disease_desciption])

    disease_precautions = precautions[precautions['Disease'] == pred_dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
    disease_precautions = [col for col in disease_precautions.values]

    disease_medications = medications[medications['Disease'] == pred_dis]['Medication']
    disease_medications = [med for med in disease_medications.values]

    disease_diet = diets[diets['Disease'] == pred_dis]['Diet']
    disease_diet = [die for die in disease_diet.values]

    disease_workout = workout[workout['disease'] == pred_dis] ['workout']
...
def predicted_value(patient_symptoms):
    i_vector = np.zeros(len(symptoms_dict))
    for i in patient_symptoms:
        i_vector[symptoms_dict[i]] = 1
